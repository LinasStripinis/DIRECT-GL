function [minima, xatmin, history] = dDirect_GLc(Problem, bounds, opts)
%--------------------------------------------------------------------------
% Function   : dDirect_GLc
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = dDirect_GLc(Problem, bounds, opts)
...
%--------------------------------------------------------------------------
    ...
    [VAL, MSS, CE] = Calulcs(VAL, Problem, MSS, CE, third, POH, Main,...
        SS, 2);
    
    % Find minima values
    [MV, MD, minval, xatmin] = Find_min(CE, MSS);
...
getopts(opts, 'maxits', 1000, 'maxevals', 100000, 'maxdeep', 1000,...
    'testflag', 0, 'globalmin', 0, 'ept', 0, 'tol', 0.01, 'showits', 1);
...
function [VAL, MSS, CE] = Calulcs(VAL, Problem, MSS, CE, third,...
    POH, Main, SS, P)
%--------------------------------------------------------------------------
[MSS, CE, VAL] = STORAGE(Problem, MSS, CE, third, Main, VAL, SS,...
    P);
...
%--------------------------------------------------------------------------
function [MSS, CE, VAL] = STORAGE(Problem, MSS, CE, third, MN, VAL, SS, P)
%--------------------------------------------------------------------------
for i = 1:size(MN, 2)
...
A.cR  = A.cL;
A.lL  = A.LO*ones(1, S);
[A.L, A.R, A.gL, A.gR, A.eL, A.eR, A.fL, A.fR] = deal(zeros(1, S));
...
for g = 1:S
    ...
    % right side
    if A.gL(g) == 0, A.fL(g) = 0; else, A.fL(g) = 1; end
    ...
    if A.gR(g) == 0, A.fR(g) = 0; else, A.fR(g) = 1; end
end
if P == 2
...
%--------------------------------------------------------------------------
function [MV, MD, minval, xatmin] = Find_min(CE, MSS)
%--------------------------------------------------------------------------
...
for i = 1:size(MSS, 2)
    if CE(i) ~= 0
        ...
        TM = find(MSS(i).E(1:CE(i)) == max(MSS(i).E(Temp == min(Temp))));
        MV(1, i) = Temp(TM);
        MV(2, i) = TM;
        MV(3, i) = MSS(i).E(TM);
