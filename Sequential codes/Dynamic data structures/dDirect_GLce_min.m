function [minima, xatmin, history] = dDirect_GLce_min(Problem, bounds, opts)
%--------------------------------------------------------------------------
% Function   : dDirect_GLce_min
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = dDirect_GLce_min(Problem, bounds, opts)
...
%--------------------------------------------------------------------------
    ...
    [VAL, SS, Xminval, minval] = Arewedone(MSS, minval, VAL, SS,...
        Xminval, Problem, CE);
...
getopts(opts, 'maxits', 1000, 'maxevals', 100000, 'maxdeep', 1000,...
    'testflag', 0, 'globalmin', 0, 'ept', 0.00000001, 'tol', 0.01, 'showits', 1);
...
SS.TESTflag  = testflag; % terminate if global minima is known
...
VAL.tet     = 10^(-6);
VAL.ep      = 0.0001;
VAL.CARD    = 10*(size(bounds(:, 1), 1)^3); % infeas HR
VAL.CARDi   = 1000*(size(bounds(:, 1), 1)^3);
...
[MSS(1).F(1), minval, VAL.min_last, VAL.fminloc,...
...
[VAL.xatmin, xatmin] = deal(MSS(1).C(:, 1));
Xminval = (abs(VAL.b - VAL.a)).*xatmin(:,1) + VAL.a;
...
VAL.xatmin   = xatmin;
[minval, VAL.min_last, VAL.fminloc, VAL.min_gen] = deal(minval);
% Check stop condition if global minima is known
[VAL, ~, Xminval, minval] = Arewedone(MSS, minval, VAL, SS, Xminval,...
    Problem, CE);
%--------------------------------------------------------------------------
return
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
function [VAL, SS, Xminval, minval] = Arewedone(MSS, minval, VAL, SS,...
    Xminval, Problem, CE)
%--------------------------------------------------------------------------
if (abs(minval - VAL.min_last) < 0.01*abs(minval))
...
[minval, VAL.min_gen] = deal(min(minval, VAL.min_gen));
if (VAL.stagnate > 0) && (abs(VAL.min_gen - VAL.fminloc) >...
        0.01*abs(VAL.min_gen))
    
    options = optimoptions('fmincon', 'StepTolerance', SS.TOL/1000);
    [xminloc, VAL.fminloc, ~, output] = fmincon(Problem.f, Xminval, [],...
...
if VAL.fcount + VAL.nFunc > SS.MAXevals % Have we exceeded the maxevals?
...
if SS.G_nargout == 3                     % Store History
    ...
    VAL.history(VAL.itctr,2) = VAL.fcount + VAL.nFunc;
    VAL.history(VAL.itctr,3) = minval;
    VAL.history(VAL.itctr,4) = VAL.time;
end

VAL.itctr = VAL.itctr + 1;               % Update iteration number
%--------------------------------------------------------------------------
return
...
Xminval = (abs(VAL.b - VAL.a)).*Xminval(:, 1) + VAL.a;
