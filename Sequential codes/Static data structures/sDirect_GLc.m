function [minima, xatmin, history] = sDirect_GLc(Problem, bounds, opts)
%--------------------------------------------------------------------------
% Function   : sDirect_GLc
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = sDirect_GLc(Problem, bounds, opts)
...
S = Find_poh(MSS.FF(1:VAL.I) + MSS.XX(1:VAL.I) + MSS.df(1:VAL.I),...
    MSS.DD(1:VAL.I), MSS.II(1:VAL.I));
...
[MSS.FF(1), Fmin] = deal(feval(Problem.f, Point));
...
%--------------------------------------------------------------------------
        ...
        if Fmin == 0
            ...
            m_idx = index(m_idx);
            fminindex = m_idx(find(MSS.DD(m_idx) == max(MSS.DD(m_idx)),...
                        1, 'last'));
            Xmin = MSS.CC(:, fminindex);
            id = 1;
...
for i=1:POH
    ...
    cor{i} = transpose(arrayfun(@(x) CallC(O, point_r(:,x),...
        OPTI), (1:LL(i)).'));
    f_l{i} = transpose(arrayfun(@(x) feval(O.f, point_l(:,x)),...
        (1:LL(i)).'));
    f_r{i} = transpose(arrayfun(@(x) feval(O.f, point_r(:,x)),...
        (1:LL(i)).'));
    if P == 2
