function [minima, xatmin, history] = sDirect_GLce(Problem, bounds, opts)
%--------------------------------------------------------------------------
% Function   : sDirect_GLce
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = sDirect_GLce(Problem, bounds, opts)
...
%--------------------------------------------------------------------------
    ...
    [VAL, Fmin, Xmin, MSS, XFmin] = Arewedone(OPTI, VAL, MSS);
...
minima      = Fmin;
if OPTI.G_nargout == 2
    xatmin    = (abs(VAL.b - VAL.a)).*XFmin(:, 1) + VAL.a;
...
MSS.FF(1)      = deal(feval(Problem.f, Point));          
MSS.XX(1)      = CallC(Problem, Point, OPTI);
Xmin           = MSS.CC(:, 1);            % initial point
XFmin          = MSS.CC(:, 1);            % initial point
if MSS.XX(1) ~= 0
    id = 0;
...
function [VAL, Fmin, Xmin, MSS, XFmin] = Arewedone(OPTI, VAL, MSS)
%--------------------------------------------------------------------------
TMf           = find(MSS.XX(1:VAL.I) == 0);
[Fmin, index] = min(MSS.FF(TMf));
XFmin         = MSS.CC(:, index);
