function [minima, xatmin, history] = sDirect_GL(Problem, bounds, opts)
%--------------------------------------------------------------------------
% Function   : sDirect_GL
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = sDirect_GL(Problem, bounds, opts)
...
while VAL.perror > OPTI.TOL                                 % Main loop
    ...
    [VAL, Fmin, Xmin] = Arewedone(OPTI, VAL, MSS, Xmin);
end                                                         % End of while
...
Euclid_dist = sum((Xmin(:, 2) - MSS.CC(:, 1:VAL.I)).^2, 1).^0.5;
...
Xmin = [MSS.CC(:, 1), MSS.CC(:, 1)];              % initial point
