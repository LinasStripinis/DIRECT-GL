function [minima, xatmin, history] = parallel_dDirect_GLce_min(Problem,...
    bounds, opts)
%--------------------------------------------------------------------------
% Function   : parallel_dDirect_GLce_min
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = parallel_dDirect_GLce_min(Problem, bounds, opts)
...
%--------------------------------------------------------------------------
    ...
    [MSS, CE, third, VAL, Q] = Alocate(bounds, SS);
    ...
    while VAL.perror > SS.TOL           % Main loop
        ...
        if labindex == 1                % Gather information from workers
            [DATA, Fmin, VAL, SS, XXMin] = MasterLAB(VAL, SS, MSS, Q, Problem);
        else                            % Receive information from master
...
getopts(opts, 'maxits', 100, 'maxevals', 100000, 'maxdeep', 100,...
    'testflag', 0, 'globalmin', 0, 'ept', 0, 'tol', 0.01,...
    'showits', 1);
...
%--------------------------------------------------------------------------
function [MSS, CE, third, VAL, Q] = Alocate(bounds, SS)
%--------------------------------------------------------------------------
...
[MSS(1).C(:, 1), VAL.xatmin] = deal(ones(VAL.n, 1)/2);

XXMin = abs(VAL.b - VAL.a).*MSS(1).C(:, 1) + VAL.a;      % Real point
[MSS(1).F(1), Fmin, VAL.min_last, VAL.fminloc,...
    VAL.min_gen] = deal(feval(O.f, XXMin));
MSS(1).cons(1) = CallConstraints(O, XXMin, SS.ept);      % g(x) eval.
...
%--------------------------------------------------------------------------
function [DATA, fmin, VAL, SS, Xreturn] = MasterLAB(VAL, SS, MSS, Q, Problem)
%--------------------------------------------------------------------------
% collect information
...
if VAL.PHASE == 1
    ...
    if (VAL.stagnate > 0) && (abs(VAL.min_gen - VAL.fminloc) >...
        ...
        options = optimoptions('fmincon', 'StepTolerance', SS.TOL/100000);
        [xminloc, VAL.fminloc, ~, output] = fmincon(Problem.f, Xreturn, [],...
...
if VAL.fcn + VAL.nFunc > SS.MAXevals
...
if SS.G_nargout == 3
    ...
    SS.history(VAL.itctr, 2) = VAL.fcn + VAL.nFunc;
