function [minima, xatmin, history] = parallel_dDirect_GLc(Problem,...
    bounds, opts)
%--------------------------------------------------------------------------
% Function   : parallel_dDirect_GLc
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = parallel_dDirect_GLc(Problem, bounds, opts)
...
% work for master
DATA = {KK, TT, I, size(I, 1), DEL, VAL.xatmin, VAL.PHASE, Fmin, 0};
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
% collect information
[count, LB, CC, FC, EE, MM]  = deal(ones(1, SS.workers));
[VR, IR, ER, DV, DI, DE] = deal(nan(SS.workers, VAL.aloc));
...
for i = 1:SS.workers
    ...
    XR(:, i)          = Q{3}(:, 1);           % x
    DV(i, 1:count(i)) = Q{5}(1, :);
    DI(i, 1:count(i)) = Q{5}(2, :);
    DE(i, 1:count(i)) = Q{5}(3, :);
end
zz = max(count);

[MSV, MSD] = deal(nan(4, zz));
...
if VAL.PHASE == 0
    ...
    [KK, TT, I, DEL, fcnc] = Decision(POH_G, POH_L, VAL, MSD, MSV, SS);
    DATA = {KK, TT, I, size(I, 1), DEL, xmin, VAL.PHASE, fmin, VAL.epsil};
end
...
[MSS, CE] = STORAGE(Probl, MSS, CE, third, VAL, DT, MAIN, SS, Q{5}, Q{1});
...
for i = 1:size(MSS, 2)
    PH = [];
    if size(find(D_O{1} == i), 2) ~= 0 && size(find(D_O{2} == i), 2) ~= 0
        PH = [MV(2, i), MD(2, i)];
    elseif size(find(D_O{1} == i), 2) ~= 0
        PH = MV(2, i);
    elseif size(find(D_O{2} == i), 2) ~= 0
        PH = MD(2, i);
    end
    ...
    if CE(i) == length(PH)
        ...
        MSS(i).cons(1:length(C)) = MSS(i).cons(C);
...
% Create empty set
TT = nan(3, size(MSS, 2));
Q = {TT, nan, zeros(n, 1), nan, TT, nan(2, 1)};
