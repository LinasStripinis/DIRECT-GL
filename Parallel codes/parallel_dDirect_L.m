function [ret_minval, final_xatmin, history] = parallel_dDirect_L...
    (Problem, bounds, opts)
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
    ...
    [MD, MSS, CE, third, VAL, SS] = Alocate(bounds, SS);
    
    if labindex == 1 
        [MD, MSS, CE, DT, VAL, SS, xmin, fmin] = Initialization(Problem,...
            MD, MSS, CE, VAL, SS);
    else        
        DT = labReceive(1);
    end
    while VAL.perror > SS.TOL       % Main loop
        [MSS, CE, MD, DT] = subdivision(VAL, Problem, DT, MSS, CE,...
            third, MD);
...
%--------------------------------------------------------------------------
function [MD, MSS, CE, third, VAL, SS] = Alocate(bounds, SS)
%--------------------------------------------------------------------------
...
MD = nan(3,SS.MAXdeep);     % for min values in collums
...
%--------------------------------------------------------------------------
function [MD, MSS, CE, DATA, VAL, SS, xmin, fmin] = Initialization(...
    Problem, MD, MSS, CE, VAL, SS)
%--------------------------------------------------------------------------
% Create and allocating empty sets
SS.history = zeros(SS.MAXdeep, 4);              % allocating history
MD(1:3,1) = deal(1);                            % first fake value
MSD = ones(4, 1);
...
[KK, TT, I, DEL, VAL.fcount] = DIVas(1, VAL, MSD, SS);
% work for master
...
%--------------------------------------------------------------------------
function [MSS, CE, MD, DATA] = subdivision(VAL, Problem, DT, MSS, CE,...
    third, MD)
%--------------------------------------------------------------------------
% Create sets
...
% Find minima values
[MD, xmin, fmin] = find_min(CE, MSS, VAL.n, DT{6});
DATA = {MD, xmin, fmin};
%--------------------------------------------------------------------------
labBarrier
...
%--------------------------------------------------------------------------
function [MSS, CE] = STORAGE(Problem, MSS, CE, third, VAL, DATA, MAIN, MD)
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
function [MD, xmin, fmin] = find_min(CE, MSS, n, XX)
%--------------------------------------------------------------------------
if size(MSS, 2) ~= 1
    % Create sets
    [MD, temp] = deal(nan(3, size(MSS, 2)));
    ...
    fmin(1) = min(temp(1, :));
    Least = find(temp(3, :) == max(temp(3, (temp(1, :) == fmin))));
    xmin = MSS(Least).C(:,temp(2, Least));
    fmin(2) = temp(3, Least);
else
    xmin = zeros(n, 1);
    fmin = nan(1, 2);
...
[KK, TT, I, DEL, fcnc] = DIVas(POH_L, VAL, MSD, SS);
DATA = {KK, TT, I, size(I, 1), DEL, xmin};
